...
function setup() {
  ...
  const market = { preflight: async () => {}, tradable: async () => true, snapshot: vi.fn(async () => snapshot(0n, 0n, 0n, 0n, 100n)) };
...
it('ignores overlapping ticks and checks stop again after the market read', async () => {
  ...
  await s.worker.tick(); await s.worker.stop(); release(); await first;
