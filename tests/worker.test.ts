...
function setup() {
	...
	const market = {
		...
		tradable: async () => true,
		snapshot: vi.fn(async () => snapshot(0n, 0n, 0n, 0n, 100n)),
	};
	const execute = vi.fn(async () => "settled" as const),
		reconcile = vi.fn(async () => "pending" as const),
		readOwner = vi.fn(async (): Promise<Address> => wallet);
...
it("reconciles pending execution before obtaining any new snapshot", async () => {
	...
	const j = await s.store.load();
	j.pending = { kind: "fusion", id: "pending", payload: {} };
	await s.store.save(j);
	await s.worker.tick();
	expect(s.reconcile).toHaveBeenCalledOnce();
	expect(s.market.snapshot).not.toHaveBeenCalled();
	expect(s.execute).not.toHaveBeenCalled();
});
it("never resumes live pending execution when configured for simulation", async () => {
	...
	expect(s.execute).not.toHaveBeenCalled();
	expect((await s.store.load()).pending?.id).toBe("pending");
	expect(s.worker.status().state).toBe("pending");
});
it("erases keys and pauses on owner transfer; price errors do not kill later cycles", async () => {
	...
	s.readOwner.mockResolvedValue(
		"0x2222222222222222222222222222222222222222" as typeof wallet,
	);
	await s.worker.tick();
	expect(s.cfg.credentials.oneinch).toBeUndefined();
	expect(s.worker.status().state).toBe("paused");
...
it("ignores overlapping ticks and checks stop again after the market read", async () => {
	...
	await s.worker.configure({ oneinch: "synthetic", mode: "live" }, wallet);
	let release!: () => void;
	s.market.snapshot.mockImplementationOnce(async () => {
		await new Promise<void>((r) => {
			release = r;
		});
		return snapshot(0n, 0n, 0n, 0n, 100n);
	});
	...
	await s.worker.stop();
	let drained = false;
	const drain = s.worker.drain().then(() => {
		drained = true;
	});
	...
	expect(s.market.snapshot).toHaveBeenCalledOnce();
	expect(s.execute).not.toHaveBeenCalled();
});
it("a completed stop invalidates an earlier configuration still awaiting preflight", async () => {
	const s = setup();
	let release!: () => void;
	s.market.preflight.mockImplementationOnce(
		() =>
			new Promise<void>((r) => {
				release = r;
			}),
	);
	const configure = s.worker.configure(
		{ oneinch: "synthetic", mode: "live" },
		wallet,
	);
	...
	release();
	await failed;
	expect(s.worker.status().state).toBe("stopped");
	expect(s.cfg.credentials.oneinch).toBeUndefined();
	await s.worker.tick();
	expect(s.execute).not.toHaveBeenCalled();
});
