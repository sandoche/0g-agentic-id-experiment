import { type AgentClient, AgenticID } from "@0gfoundation/0g-agenticid-sdk";
...
it("activates once, verifies readiness, then configures keys outside chat on both calls", async () => {
	...
	const mock = vi.spyOn(AgenticID, "fromAttestor").mockResolvedValue({
		agent: { client: async () => client, getAgentSeal: async () => wallet },
		reputation: { verifyProof: async () => ({ ok: true }) },
	} as unknown as AgenticID);
	const proofMock = vi
		.spyOn(proofModule, "createProofVerifier")
		.mockResolvedValue(async () => true);
	try {
		const cfg = {
			...config,
			credentials: { ownerKey: key, oneinch: "synthetic-secret" },
		};
		...
		expect(JSON.stringify(chat.mock.calls)).not.toContain(
			config.strategy.prompt,
		);
		started = false;
		await activate(cfg, 1n, "fixture");
		expect(chat).toHaveBeenCalledTimes(2); // reset reconstructs without strategy in the message
	} finally {
		mock.mockRestore();
		proofMock.mockRestore();
	}
});
