...
import { config, holding, snapshot, tao, zero, usdBase, usdB, usdR } from './fixtures.js';
const strategy = config.strategy;

test('values 6-decimal holdings exactly beyond Number precision', () => {
  expect(holdingValue({ ...holding(usdBase, 0n, 250000000n), units: 9007199254740993n }))
    .toBe(2251799813685248250n);
});
test('sells overweight tokens before buying deficits', () => {
  expect(nextAction(strategy, snapshot(800n, 200n))).toEqual({ kind: 'swap', from: tao, to: usdR, amount: 200n * 10n ** 18n });
});
test('an exactly balanced portfolio stays idle', () => {
  expect(nextAction(strategy, snapshot(600n, 400n))).toEqual({ kind: 'idle', reason: 'WITHIN_THRESHOLDS' });
});
...
test('prices settlement cash at its actual value', () => {
  ...
  expect(nextAction(strategy, s)).toEqual({ kind: 'transfer', from: usdBase, to: usdR, amount: 600000000n });
});
test('rounds token units down and never overspends', () => {
  ...
  expect(result).toEqual({ kind: 'transfer', from: usdBase, to: usdR, amount: 60600000n });
});
test('leaves small deposits and rounding dust unspent', () => {
  expect(nextAction(strategy, snapshot(600n, 400n, 0n, 0n, 1n)).kind).toBe('idle');
});
test('does not liquidate small drift below its threshold', () => {
  expect(nextAction(strategy, snapshot(601n, 399n)).kind).toBe('idle');
});
test.each(['incomplete', 'missing', 'stale', 'future', 'negative', 'zero-price', 'duplicate', 'native'])('halts on %s data', problem => {
  const s = snapshot(600n, 400n);
  if (problem === 'incomplete') s.complete = false;
  if (problem === 'missing') s.holdings = s.holdings.filter(h => h.asset.id !== zero.id);
  ...
  if (problem === 'native') s.holdings.push(holding({ ...tao, id: 'native', symbol: 'ETH' }, 100n));
