...
const files = {
	"worker.mjs": "export const fixture = true;",
	"package.json": "{}",
	"package-lock.json": "{}",
};
it("retries an interrupted dependency install and marks only a successful matching lock", () => {
	...
	const fs = {
		...
		existsSync: (p: string) => p === "node_modules" || disk.has(p),
		mkdirSync: () => {},
		writeFileSync: (p: string, v: string) => disk.set(p, v),
				...
				name === "node:fs"
					...
					: name === "node:crypto"
						...
						: name === "node:child_process"
							? { spawnSync: install, spawn }
							: {},
		});
	expect(run).toThrow("DEPENDENCY_INSTALL_FAILED");
	...
	run();
	expect(install).toHaveBeenCalledTimes(2);
	disk.set("package-lock.json", '{"changed":true}');
	run();
...
it("preserves validated runtime infrastructure settings without embedding credentials", () => {
	const cfg = {
		...
		rpcUrls: {
			56: "https://bnb.example",
			4663: "https://rh.example",
			8453: "https://base.example",
			42161: "https://arb.example",
		},
		credentials: { oneinch: "synthetic-private-trading-key" },
	};
	const payload = unpackCapability(
		buildCapability(cfg.strategy, files, cfg).systemPrompt,
	);
	const restored = readConfig(JSON.parse(payload["runtime.json"]!));
	expect(restored.attestorUrl).toBe(cfg.attestorUrl);
	expect(restored.rpcUrls).toEqual(cfg.rpcUrls);
	expect(restored.model).toBe(cfg.model);
	expect(JSON.stringify(payload)).not.toContain(
		"synthetic-private-trading-key",
	);
});
it("rejects checksum corruption, traversal and oversized files", () => {
	const cap = buildCapability(config.strategy, files);
	expect(() =>
		unpackCapability(cap.systemPrompt.replace(cap.sha256, "0".repeat(64))),
	).toThrow("PAYLOAD_CHECKSUM");
	expect(() =>
		buildCapability(config.strategy, { ...files, "../.env": "secret" }),
	).toThrow("UNSAFE_PAYLOAD_PATH");
	expect(() =>
		buildCapability(config.strategy, {
			...files,
			"worker.mjs": "x".repeat(9 * 1024 * 1024),
		}),
