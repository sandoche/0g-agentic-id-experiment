...
import type { AgenticID, ServeProof } from "@0gfoundation/0g-agenticid-sdk";
...
it("rejects empty, subset, duplicate, stale framework and wrong submitter bindings", async () => {
	const hash = `0x${"11".repeat(32)}` as const,
		other = `0x${"22".repeat(32)}` as const,
		submitter = "0x0000000000000000000000000000000000000000" as const;
	...
	for (const changed of [
		{ dataHashes: [] },
		{ dataHashes: [hash] },
		{ dataHashes: [hash, hash] },
		{ frameworkHash: other },
		{ submitter: `0x${"33".repeat(20)}` },
	]) {
		expect(
			bindings.validBindings({ ...proof, ...changed } as ServeProof, expected),
		).toBe(false);
	}
	expect(
		bindings.validBindings(proof, { ...expected, frameworkAllowed: false }),
	).toBe(false);
});
it("pins only a signature-verified registry-approved framework and rechecks current data", async () => {
	...
	const hash = `0x${"11".repeat(32)}` as const,
		changed = `0x${"22".repeat(32)}` as const;
	const proof: ServeProof = {
		agentId: 1n,
		timestamp: 1n,
		deadline: 2n,
	...
	const ag = {
		reputation: { verifyProof: async () => ({ ok: authentic }) },
		agent: { intelligentDatasOf: async () => [{ dataHash: current }] },
		...
		.mockImplementation(async (url, init) => {
			if (String(url).endsWith("/config"))
				return Response.json({
					...
					agentic_id_addr: `0x${"44".repeat(20)}`,
				});
			...
			return Response.json({
				...
				result:
					rpc.method === "eth_chainId"
						? "0x40da"
						: `0x${(approved ? "1" : "0").padStart(64, "0")}`,
			});
		});
	try {
		const verify = await bindings.createProofVerifier(
			config,
			ag,
			1n,
			undefined,
...
it("binds method, exact URI, exact response bytes, status, identity and cryptographic verification", async () => {
	...
	await expect(
		verifyTranscript(p, 1n, path, body, 200, async () => true),
	).resolves.toBeUndefined();
	for (const altered of [
		{ ...p, agentId: 2n },
		{ ...p, deadline: now - 1n },
		{ ...p, taskHash: `0x${"00".repeat(32)}` },
		{ ...p, timestamp: now + 60n },
	]) {
		await expect(
			verifyTranscript(
				altered as ServeProof,
				1n,
				path,
				body,
				200,
	...
	await expect(
		verifyTranscript(p, 1n, path, body, 200, async () => false),
	).rejects.toThrow("PROOF_INVALID");
	await expect(
		verifyTranscript(p, 1n, "/api/events?after=2", body, 200, async () => true),
	).rejects.toThrow("PROOF_INVALID");
});
