...
const desc = {
	srcToken: usdR.address,
	dstToken: tao.address,
	srcReceiver: other,
	dstReceiver: wallet,
...
it("can obtain preview calldata before approval while local signing still estimates execution", async () => {
	...
	await createSwapApi(
		{ ...config, credentials: { oneinch: "synthetic" } },
		wallet,
		fetcher,
	).swap(action);
});
describe("rejects unsafe calldata before any signer is called", () => {
	it.each([
		{ dstReceiver: other },
		{ srcToken: tao.address },
		{ dstToken: usdR.address },
		{ amount: 1n },
		{ minReturnAmount: 994n },
		{ flags: 1n },
		{ flags: 2n },
		{ flags: 4n },
	])("rejects mutation %#", (change) => {
		expect(() =>
			validateSwap(action, wallet, 4663, ROUTER, response(change), 1000n, 50n),
		).toThrow();
	});
	it("rejects wrong chain, spender, router, sender, value and selector", () => {
		expect(() =>
			validateSwap(action, wallet, 1, ROUTER, response(), 1000n, 50n),
		).toThrow();
		expect(() =>
			validateSwap(action, wallet, 4663, other, response(), 1000n, 50n),
		).toThrow();
		for (const change of [
			{ to: other },
			{ from: other },
			...
			const r = response();
			Object.assign(r.tx, change);
			expect(() =>
				validateSwap(action, wallet, 4663, ROUTER, r, 1000n, 50n),
			).toThrow();
		}
	});
});
...
it("persists signed bytes before broadcast; recovers an accepted timeout without signing twice", async () => {
	const store = state(),
		signed = "0x1234" as const;
	let mined = false;
	const port = {
		sign: vi.fn(async () => signed),
		receipt: vi.fn(async () => (mined ? ("success" as const) : undefined)),
		broadcast: vi.fn(async () => {
			expect((await store.load()).pending?.id).toBe(keccak256(signed));
			throw new Error("timeout");
		}),
	};
	const runner = createTransactionRunner(store, port);
	expect(
		await runner.send({ chainId: 4663, to: ROUTER, data: "0x", value: 0n }),
	).toBe("pending");
	expect(
		await runner.send({ chainId: 4663, to: ROUTER, data: "0x", value: 0n }),
	).toBe("pending");
	mined = true;
	expect(await createTransactionRunner(store, port).reconcile()).toBe(
		"settled",
	);
...
it("keeps unsigned/broadcast-free state when saving fails", async () => {
	...
	const runner = createTransactionRunner(store, {
		sign: async () => "0x1234",
		broadcast,
		receipt: async () => undefined,
	});
	await expect(
		runner.send({ chainId: 56, to: ROUTER, data: "0x", value: 0n }),
...
it("recovers a crash before broadcast by resending only the persisted signed bytes", async () => {
	...
	expect(await runner.reconcile()).toBe("pending");
	expect(await runner.reconcile()).toBe("pending");
	expect(sign).not.toHaveBeenCalled();
	expect(broadcast).toHaveBeenCalledTimes(2);
	expect(broadcast.mock.calls[0]).toEqual([56, raw]);
	expect(broadcast.mock.calls[1]).toEqual([56, raw]);
...
it("validates response before requesting an allowance signature", async () => {
	...
	const swap = createSwap(
		...
		{
			quote: async () => 1000n,
			swap: async () => response({ dstReceiver: other }),
			spender: async () => ROUTER,
		},
		{
			verifyRouter: async () => {},
			balance: async () => action.amount,
			allowance: async () => 0n,
			guard: async () => {},
			send: signer,
			approve,
