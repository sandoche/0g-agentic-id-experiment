...
async function server() {
	const configure = vi.fn(async () => {}),
		stop = vi.fn(async () => {});
	const worker = {
		status: () => ({ mode: "simulation", state: "ready" }),
		events: () => [],
		configure,
		stop,
		start: vi.fn(),
	...
	const s = await startServer(
		worker,
		createAuth(identity, async () => owner.address),
		identity,
		"abc",
	);
...
it("serves only exact routes, bounded public fields and correctly signed configuration", async () => {
	...
	expect(await (await fetch(`${s.url}/api/status`)).json()).toEqual({
		...identity,
		checksum: "abc",
		mode: "simulation",
		state: "ready",
	...
	const payload = JSON.stringify({
		oneinch: "synthetic-secret",
		mode: "simulation",
	});
	const signature = await owner.signMessage({
		message: authMessage("configure", challenge, payload),
	});
	const r = await fetch(`${s.url}/api/configure`, {
		method: "POST",
		body: JSON.stringify({ challenge, payload, signature }),
	});
	expect(r.status).toBe(200);
	expect(s.configure).toHaveBeenCalledWith(JSON.parse(payload), owner.address);
...
it("rejects overflow, unknown fields, and wrong signatures without echoing secrets", async () => {
	...
	const r = await fetch(`${s.url}/api/configure`, {
		method: "POST",
		body: "private-secret".repeat(2000),
	});
	expect(r.status).toBe(413);
	expect(await r.text()).not.toContain("private-secret");
	const challenge = await (await fetch(`${s.url}/api/challenge`)).json();
	const payload = '{"url":"private-secret"}',
		signature = await owner.signMessage({
			message: authMessage("configure", challenge, payload),
		});
	expect(
		(
			await fetch(`${s.url}/api/configure`, {
				method: "POST",
				body: JSON.stringify({ challenge, payload, signature }),
			})
		).status,
...
it("registers exact methods and loopback backends via the sign socket transport", async () => {
	...
	expect(
		serviceDefinitions(8081).every(
			(s) => s.backend === "http://127.0.0.1:8081",
		),
	).toBe(true);
});
