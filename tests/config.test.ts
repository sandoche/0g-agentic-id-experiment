...
test('parses weights exactly without requiring keys for simulation', () => {
  ...
  expect(config.strategy.targets.map((t: {weightBps: bigint}) => t.weightBps)).toEqual([6000n, 4000n]);
  expect(config.model).toBe('glm-5.3');
});
test.each([1, 16602, 46630, 0, NaN])('rejects forbidden trading chain %s', chain => {
  expect(() => requireTradingChain(chain)).toThrow('CHAIN_NOT_ALLOWED');
});
test.each(['1e3', '-1', 'NaN', 'Infinity', '1.000000001', '0x10', ''])('rejects invalid USD %s', value => {
...
test.each([
  { FUNDING_CHAIN_ID: '1' }, { FUNDING_CHAIN_ID: '56' },
  { STRATEGY_PROMPT: '' }, { TRADING_MODE: 'automatic' },
  { REBALANCE_INTERVAL_MS: '1' }, { MAX_SLIPPAGE_BPS: '501' },
  { MAX_SLIPPAGE_BPS: '0' }, { MIN_TRADE_USD: '0' },
  { DRIFT_THRESHOLD_BPS: '-1' }, { OWNER_PRIVATE_KEY: 'secret-test-key' },
  ...
  try { readConfig({ ...input, ...override }); } catch (error) {
    expect(String(error)).not.toContain('secret-test-key');
  }
});
test.each([
  [{ symbol: 'TAO', chainId: 4663, weightBps: 5000 }],
  [{ symbol: 'TAO', chainId: 4663, weightBps: 6000 }, { symbol: 'TAO', chainId: 4663, weightBps: 4000 }],
...
].map(allocations => ({ allocations })))('rejects invalid or removed positions', ({ allocations }) => {
  expect(() => readConfig({ ...input, PORTFOLIO_ALLOCATIONS: JSON.stringify(allocations) })).toThrow();
});
test('requires both opt-in and keys before live configuration', () => {
  expect(() => readConfig({ ...input, TRADING_MODE: 'live' })).toThrow('MISSING_TRADING_KEY');
});
