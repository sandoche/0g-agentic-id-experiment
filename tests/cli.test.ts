...
it('selectively loads allowed keys and never includes the owner key for read-only commands', async () => {
  const d = await mkdtemp(join(tmpdir(), 'portfolio-cli-')), path = join(d, '.env');
  try {
    const text = 'OWNER_PRIVATE_KEY=private-synthetic\nONEINCH_API_KEY=api-synthetic\nSTRATEGY_PROMPT="Synthetic prompt"\n'; await writeFile(path, text);
    const env = await loadEnvironment(path, false); expect(env.OWNER_PRIVATE_KEY).toBeUndefined(); expect(env.ONEINCH_API_KEY).toBe('api-synthetic');
...
it('runs the CLI without real keys and fails unknown commands or missing configuration', async () => {
  ...
  const run = (args: string[], env = {}) => exec(process.execPath, [tsx, cli, ...args], { cwd: d, env: { ...process.env, ...env }, windowsHide: true });
  try {
    await expect(run(['unknown'])).rejects.toThrow();
    await expect(run(['check'])).rejects.toThrow();
    const sample = await readFile('.env.example', 'utf8'); await writeFile(join(d, '.env'), sample);
    const { stdout } = await run(['simulate', '--once'], { TRADING_MODE: 'live' });
    expect(stdout).toContain('"type":"simulation"'); expect(stdout).not.toContain('"type":"error"'); expect(stdout).not.toContain('PRIVATE_KEY');
