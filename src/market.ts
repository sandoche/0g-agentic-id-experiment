...
export async function requestJson<T>(
	...
	for (let attempt = 0; attempt < attempts; attempt++) {
		...
		try {
			...
			await delay(250 * 2 ** attempt, undefined, {
		...
		if (!response.ok) {
			if (
				...
				const ms = Number.isFinite(seconds)
					? Math.min(5000, Math.max(0, seconds * 1000))
					: response.status === 429
						? 1100
						: 250 * 2 ** attempt;
...
export function parsePrice(value: unknown): bigint {
	...
	const parsed = parseUsd(
		fraction ? `${whole}.${fraction.slice(0, 8)}` : whole!,
	);
...
export function createMarket(
	...
	const chains = [
		...new Set([
			...
			...positions.map((a) => a.chainId),
		]),
	];
	const tracked = [...positions, ...chains.map(cashAsset)];
	const rpc = new Map(
		chains.map((chain) => [chain, createRpc(config, chain, fetcher)]),
	);
	const stockRegistry = async () => {
		const data = await requestJson<{ assets: Stock[] }>(
			new URL("https://api.robinhood.com/rhj/assets"),
			{},
			fetcher,
		);
	...
	const checkStock = (asset: Asset, registry: Stock[]) => {
		...
		if (
			!stock ||
			stock.tokenDecimals !== asset.decimals ||
			!stock.deployments?.some(
				(d) =>
					d.chainId === asset.chainId &&
					d.contractAddress.toLowerCase() === asset.address,
			)
		)
			throw new Error("STOCK_METADATA_MISMATCH");
		return stock;
	};
	return {
		async preflight() {
			try {
				for (const chain of chains)
					if ((await rpc.get(chain)!.getChainId()) !== chain)
				...
				for (const asset of tracked) {
					...
					if (decimals !== asset.decimals || symbol !== asset.symbol)
		...
		async snapshot(wallet) {
			...
			for (const chain of chains) {
				...
				try {
					if ((await client.getChainId()) !== chain) throw new Error();
				} catch {
					throw new Error("RPC_CHAIN_MISMATCH");
				}
				const chainAssets = tracked.filter((a) => a.chainId === chain);
				const url = new URL(
					`https://api.1inch.com/price/v1.1/${chain}/${chainAssets.map((a) => a.address).join(",")}`,
				);
		...
		async tradable(asset) {
			...
			const data = await requestJson<{
				...
				new URL(`https://api.robinhood.com/rhj/prices/${asset.symbol}`),
				{},
				fetcher,
			);
			const quote = data.quotes?.find((q) => q.tokenSymbol === asset.symbol);
			if (quote?.isTradingHalt !== false) return false;
