...
import { safeEvent, type Event } from './log.js';
import type { Action, Address, Config } from './types.js';
import type { Market } from './market.js';
import type { Store } from './state.js';
import type { ExecutionResult } from './transaction.js';
export type Execution = { execute(action: Exclude<Action, { kind: 'idle' }>): Promise<ExecutionResult>; reconcile(): Promise<ExecutionResult> };
...
export function validateConfiguration(value: unknown): Configuration {
  ...
  if (!c || typeof c !== 'object' || Object.keys(c).sort().join(',') !== 'mode,oneinch' || typeof c.oneinch !== 'string'
    || c.oneinch.length < 1 || c.oneinch.length > 2048 || !['live', 'simulation'].includes(c.mode)) throw new Error('INVALID_CONFIGURATION');
  return c;
}
export function createWorker(config: Config, market: Market, execution: Execution, store: Store, readOwner: () => Promise<Address>, logger: (event: Event) => Promise<unknown>) {
  ...
  async function emit(event: Event) {
    ...
    const safe = safeEvent(event as unknown as Record<string, unknown>); events.push(safe); if (events.length > 300) events.shift(); await logger(safe);
  }
  async function guard() {
    if (!active) throw new Error('NOT_CONFIGURED');
    let owner: Address;
    try { owner = await readOwner(); } catch { state = 'paused'; throw new Error('OWNER_UNAVAILABLE'); }
    if (owner.toLowerCase() !== (await store.load()).owner.toLowerCase()) {
      config.credentials = {}; active = false; state = 'paused'; throw new Error('OWNER_CHANGED');
    }
    if (!active) throw new Error('NOT_CONFIGURED');
  }
  const worker = {
    guard, emit,
    status: () => ({ mode: config.mode, state, ...(lastCycleId ? { lastCycleId } : {}), ...(pendingId ? { pendingId } : {}) }),
    ...
    async tick() {
      ...
      try {
        ...
        if (config.mode === 'simulation') {
          ...
          if (!j.paper) {
            j.paper = Object.fromEntries(s.holdings.map(h => [h.asset.id, h.units]));
            if (s.holdings.every(h => h.units === 0n)) { const cash = cashAsset(config.strategy.fundingChain); j.paper[cash.id] = 1000n * 10n ** BigInt(cash.decimals); }
            await store.save(j);
          }
          for (const h of s.holdings) h.units = j.paper[h.asset.id] ?? 0n;
        }
        const action = nextAction(config.strategy, s); await guard();
        if (action.kind === 'idle') { state = 'ready'; await emit({ type: 'idle', time: Date.now(), code: action.reason }); return; }
        if (!await market.tradable(action.from) || !await market.tradable(action.to)) { state = 'ready'; await emit({ type: 'idle', time: Date.now(), code: 'MARKET_CLOSED' }); return; }
        await guard();
        if (config.mode === 'simulation') {
          const j = await store.load(), from = s.holdings.find(h => h.asset.id === action.from.id)!, to = s.holdings.find(h => h.asset.id === action.to.id)!;
          const received = action.amount * from.priceUsd * 10n ** BigInt(to.asset.decimals) * (10000n - config.strategy.slippageBps)
            / (10n ** BigInt(from.asset.decimals) * to.priceUsd * 10000n);
          j.paper![from.asset.id] = from.units - action.amount; j.paper![to.asset.id] = to.units + received; await store.save(j);
          await emit({ type: 'simulation', time: Date.now(), chainId: action.from.chainId, code: 'SIMULATED' }); state = 'ready';
        } else { const result = await execution.execute(action); state = result === 'pending' ? 'pending' : 'ready'; pendingId = (await store.load()).pending?.id; }
      } catch (e) {
