...
export function paperMarket(config: Config, clock = Date.now): Market {
  ...
  return { preflight: async () => {}, tradable: async () => true, snapshot: async () => {
    const now = clock();
    return { now, complete: true, holdings: [...tracked, ...chains.map(cashAsset)].map(asset => ({ asset, units: 0n, priceUsd: 100000000n, observedAt: now })) };
  } };
}
